...
def compute_backbone_shapes(config, image_shape):
    ...
    assert config.BACKBONE in ["resnet50", "resnet101", "mobilenet224v1","vgg16"]
    return np.array(
...
def vgg16_graph(img_input, architecture, alpha=1.0, train_bn=True):
    assert architecture in ["vgg16"]
    # Block 1
    x = Conv2D(64, (3, 3), activation='relu', padding='same', name='block1_conv1')(img_input)
    x = Conv2D(64, (3, 3), activation='relu', padding='same', name='block1_conv2')(x)
    C1 = MaxPooling2D((2, 2), strides=(2, 2), name='block1_pool')(x)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if config.BACKBONE in ["vgg16"]:
            _, C2, C3, C4, C5 = vgg16_graph(input_image, config.BACKBONE, 
                                                alpha=1.0, train_bn=config.TRAIN_BN)
...
def vgg16_mold_image(images, config):
    return images.astype(np.float32) - config.MEAN_PIXEL

def vgg16_unmold_image(normalized_images, config):
